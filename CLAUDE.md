...
- **Editor: the `D:\dms-editor` Tiptap port is now complete (landed 2026-09-19).** The
  ...
  - **Re-theming boundary — SUPERSEDED for the full editor (2026-09-19, see the "Full editor rebuilt" bullet below); still accurate for `SectionInlineEditor`'s small toolbar.** Every toolbar button/dialog in `src/editor/toolbar/` is custom
...
- **Full editor rebuilt as a Google-Docs-style shell with the library's NATIVE toolbar (2026-09-19).** `FullPageEditor` = `EditorTopBar` (app logo + "DMS EDITOR" + doc title, Save/Close) → `NativeToolbar` (the reactjs-tiptap-editor `RichText*` controls in the POC/playground order — fonts, heading, size, colors, table… all native UI; `CodeView` deliberately omitted because raw-HTML editing destroys the `data-section-id` markers; Lock/Comments dropped) → a row of [scrolling canvas | `EditorSideRail`]. Only the canvas scrolls; app bar, toolbar and rail are fixed (`h-screen overflow-hidden` shell). The rail is a slim icon strip; its Fields button opens a 320px panel beside it (closed by default, hidden scrollbar via the `no-scrollbar` utility in `index.css`), inserting via the same `fieldPanel(insertAtCursor)` render prop (`FieldPanel` gained a `bare` prop for that). Old custom full-scope toolbar code (`Toolbar` `scope` prop, Video/Iframe/Attachment dialogs) was deleted — `toolbar/Toolbar.tsx` is now the inline editor's toolbar only. **`useDocumentBranding`** sets the popup's `document.title` ("DMS EDITOR — name · version") and favicon (app logo) while mounted.
  - **Why the canvas carries the `reactjs-tiptap-editor` class:** the library's `style.css` scopes its content styling (tables, images, hr, selection…) under that class, so without it tables render broken. That scope also paints a white bg and gives `.ProseMirror` 32px/80px padding + min-height — `pagination.css` overrides those (top/bottom padding 0, left/right = `--rm-margin-*`). `editor.css`'s own table rules opt OUT inside that scope (`:not(.reactjs-tiptap-editor .tiptap)`) and still serve `SectionInlineEditor` + `DocumentPreview`, which don't load the library CSS.
  - **tiptap-pagination-plus table gotcha (fixed):** the plugin injects `.rm-with-pagination table tbody { display: table }` with no width, and the editor's `<colgroup>` makes the outer table have one column per `<col>`, so the inner table landed in the FIRST column only — tables rendered as a tiny cluster next to empty space (also for header/footer tables). `pagination.css` forces the outer `table` to `display:block` and `tbody` to `width:100%`. The library's unlayered CSS also beats Tailwind utilities (its preflight has `img{height:auto}`), so size such things with inline styles inside the full editor (the logo does).
  - **Word-like page geometry:** header/footer sit 0.5in (48px) from the page edge (`A4_PAGINATION_OPTIONS.marginTop/Bottom` = 48; `.rm-page-header/footer` `min-height: 48px`, so the body never starts above 1in), tables in header/footer are full width with hairline borders. `DocxExportService` writes the same numbers (A4, 1140-twip side margins, 1440 top/bottom, header/footer distance 720).
  - **Visual verification technique:** the editor is hard to judge from code, so a temporary Vite entry (`harness.html` → a tiny `main.tsx` rendering `FullPageEditor`/`DocumentPreview` with sample sections; no auth/backend needed) screenshotted via `chrome --headless=new --screenshot` (and `--dump-dom` with injected computed-style probes) was used and then deleted. Re-create it the same way if the editor layout needs debugging again.
